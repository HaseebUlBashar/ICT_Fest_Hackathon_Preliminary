...
from ..services.refunds import compute_refund_amount_cents, log_refund #TAHSIN
...
_booking_write_lock = threading.Lock() #RAIMA
...
def _has_conflict(db: Session, room_id: int, start: datetime, end: datetime) -> bool:
    ...
    _pricing_warmup()
    for b in existing:
        if b.start_time < end and start < b.end_time: #RAIMA
            return True
    return False
...
def create_booking(
    ...
    if duration_hours < MIN_DURATION_HOURS or duration_hours > MAX_DURATION_HOURS: #RAIMA
    ...
    cache.invalidate_report(user.org_id) #RAIMA
...
def list_bookings(
    ...
    total = base.count()
    items = (
       base.order_by(Booking.start_time.asc(), Booking.id.asc()) #RAIMA
        .offset((page - 1) * limit)                              #RAIMA
        .limit(limit)                                            #RAIMA
        .all()
    )
    return {
...
def get_booking(
    ...
    if user.role != "admin" and booking.user_id != user.id:  #RAIMA
        raise AppError(404, "BOOKING_NOT_FOUND", "Booking not found") #RAIMA
        
    response = serialize_booking(booking)
    #response["start_time"] = iso_utc(booking.created_at)    RAIMA
    response["refunds"] = [
        {
...
def cancel_booking(
    ...
    # refund_entry = log_refund(db, booking, refund_percent)  #Tahsin
